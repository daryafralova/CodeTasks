---Найдите день с наибольшей конверсией из количества сессий в количество сессий с покупкой. Ответ укажите в формате 2025-04-20
---Таблица:
---event_time
---event_type
---product_id
...
select date(event_time) as dt
		...
		, round(count(distinct (case when event_type = 'purchase'then user_session end))*1.0 / count(distinct user_session)*100,2) as d
